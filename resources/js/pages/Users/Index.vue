...
<template>
  <AppLayout :title="'Users'" :breadcrumbs="[{ name: 'Users', href: '/admin/users' }]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- Flash Messages -->
      <div v-if="flash.success" class="mb-6 p-4 bg-green-100 text-green-700 rounded-lg">
        {{ flash.success }}
      </div>
      <div v-if="flash.error" class="mb-6 p-4 bg-red-100 text-red-700 rounded-lg">
        {{ flash.error }}
      </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ user.region || 'N/A' }}</td>
